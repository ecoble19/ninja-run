...
ECS.systems.map = function systemMap({query, entities, timeStamp, gameState}) {
    ...
    if(player.position.x / BLOCK > CHUNK_SIZE * chunksRendered / 2) {
        console.log('RENDER');
        ...
        for (let i = 0; i < map.length; i++) {
            for (let j = 0; j < map[i].length; j++) {
                if (map[i][j] === TileType.Land) {
                    spawnTile(ECS, {x: j * BLOCK + (chunksRendered * CHUNK_SIZE * BLOCK), y: WINDOW[1] - BLOCK * (i + 1)})
                } else if (map[i][j] === TileType.Coin) {
                    spawnCoin(ECS, {x: j * BLOCK  + (chunksRendered * CHUNK_SIZE * BLOCK), y: WINDOW[1] - 120 * (i + 1)})
                } else if(map[i][j] === TileType.Spike) {
                    spawnSpike(ECS, {x: j * BLOCK + (chunksRendered * CHUNK_SIZE * BLOCK), y: WINDOW[1] - 120 * (i + 1)})
                }
            }
        }
        chunksRendered++;
